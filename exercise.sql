...
-- whereas truncate delete all the rows of the table only
...
(	SELECT p.customer_id,
	...
	FROM payment p
 	JOIN rental r
 	USING(rental_id)
	WHERE amount<2.99 AND r.staff_id=1
...
SELECT customer_id,film_id, count(*) AS cnt FROM rental
JOIN inventory
USING(inventory_id)
